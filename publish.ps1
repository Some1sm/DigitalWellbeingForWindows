...
# WPF
dotnet build -c Release DigitalWellbeingWPF/DigitalWellbeingWPF.csproj
if (-not (Test-Path "$releaseDir/WPF")) { New-Item -ItemType Directory -Force -Path "$releaseDir/WPF" | Out-Null }
Copy-Item -Recurse "DigitalWellbeingWPF/bin/Release/*" "$releaseDir/WPF"
